...
Starting April 4, 2026, Anthropic will no longer allow Claude subscription limits to cover third-party harnesses like DOSRouter. If you've been using your Claude Pro/Team subscription to power autonomous agents, code assistants, or any tool outside of Anthropic's own products — that stops tomorrow.

## What Changed

Anthropic sent this to affected users:

> Starting April 4 at 12pm PT, you'll no longer be able to use your Claude subscription limits for third-party harnesses including DOSRouter. To keep using third-party harnesses with your Claude login, turn on extra usage — a pay-as-you-go option billed separately from your subscription.
...
If you're running agents through DOSRouter, Continue.dev, or any third-party harness, your costs are about to change dramatically:
...
This is exactly the problem [DOSRouter](https://github.com/DOSAI/DOSRouter) solves.

## DOSRouter: Smart Routing for Agents

DOSRouter is an open-source local proxy that sits between your agent and 55+ LLM models across 9 providers. It analyzes every request across 15 dimensions and routes it to the cheapest model that can handle it — in under 1ms, entirely locally.

```
Your Agent → DOSRouter (localhost:8402) → Best model for the job
...
```bash
npx @dos/dosrouter    # Install and start (generates wallet automatically)
```
...
- **Budget caps** — set a max spend per session, DOSRouter gracefully downgrades when budget runs low
...
DOSRouter is open source, runs locally, and takes 30 seconds to set up:
...
_DOSRouter is built by [DOS](https://dos.ai). Source code: [github.com/DOSAI/DOSRouter](https://github.com/DOSAI/DOSRouter)_
